...
def run(message, bot):
    ...
    try:
        ...
        # markup.row_width = 2
        markup.add(
            "PDF for Total Expenses - Category wise",
            "PDF showing who owes whom how much",
        )
        msg = bot.send_message(
            chat_id, "Which kind of PDF do you want to generate?", reply_markup=markup
        )
            ...
            for user, details in user_list.items():
                ...
                for i, user_name in enumerate(details["users"]):
                    ...
                    pdf.cell(
                        50,
                        10,
                        str(round(details["owed"][user_name], 2)),
                        border=1,
                        align="C",
                        fill=True,
                    )
                    pdf.cell(
                        50,
                        10,
                        ", ".join(details["owing"][user_name].keys()),
                        border=1,
                        align="C",
                        fill=True,
                    )
                    if details["owing"][user_name].values() != []:
                        pdf.cell(
                            ...
                            ", ".join(
                                [
                                    str(round(x, 2))
                                    for x in details["owing"][user_name].values()
                                ]
                            ),
                            border=1,
                            align="C",
